        ...
        if len(msg) > 0:
            ...
            if code == M_PLAY:
                ...
                if to_name == from_name:
                    ...
                    self.all_games.append([from_name,to_name])
                    game_group[game_group.grp_ever] = [from_name,to_name]
                    game_group.grp_ever += 1
                    msg = M_PLAY + 'ok'
                else:
                    msg = M_PLAY + 'no_user'
                ...
                for p in peers[1:]:
                    to_sock = self.logged_name2sock[p]
                    mysend(to_sock, msg)

            elif code == '!':   #this is the game's version of M_EXCHANGE; ran out of constants to use
                from_name = self.logged_sock2name[from_sock]
                to_name = self.game_grp.me_against(from_name)
                to_sock = self.logged_name2sock[to_name]
                board = msg
                mysend(to_sock,board)

            elif code == '$':   #disconnect from game
                from_name = self.logged_sock2name[from_sock]
                to_name = ''
                found, group_num = self.game_group.find_group(from_name)
                self.game_group.disconnect(from_name)
...
#==============================================================================
# handle message exchange: one peer for now. will need multicast later
#==============================================================================
